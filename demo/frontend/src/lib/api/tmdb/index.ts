...
export const getEpisodesBySeason = async (
  ...
  const { episodes } = await db<Api.SeasonDetails>(
...
export const getShowsByGenre = async (
  genre: Api.Genre
): Promise<Record<string, Api.TV[]>> => {
  const { results } = await db<Api.Page<Api.TV>>(
    "/discover/tv",
...
export const getShowsByGenres = async (
  genres: Api.Genre[]
): Promise<Record<string, Api.TV[]>> => {
  const result = await Promise.all(
    genres.map((genre) => getShowsByGenre(genre))
  );
